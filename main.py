...
import PIL, os, tkinter, threading, math
...
def color (n, m):
    if COL_M == 0:
        ...
        if s <= C:
            return (M1, int ((1 - (s / C)) * 255 // 1), int ((1 - (s / C)) * 255 // 1))
        elif s <= 2*C:
            ...
            return (int ((7 - (s / C)) * M1 // 1), 0, int ((7 - (s / C)) * M3 // 1))
        else:
            return (0, 0, 0)
...
def save_image (image):
    ...
    if lst:
        image.save ('rendered/' + str (int (int (sorted (lst) [-1] [:4:])) + 1).rjust (4, '0') + '.png', 'PNG')
    else:
...
def kartinka ():
    ...
    while y < int (height // 1):
        ...
        lpr ['text'] = 'Генерация изображения...    ' + q [0] + '.' + q [1].ljust (3, '0') + ' %'
        image_window.update ()
...
def video (vid):
    ...
    lpr_v = tkinter.Label (bg = color2, text = 'Генерация видео...    0 %    ( 0 / ' + str (len (vidd)) + ' )')
    ...
    for q in range (len (vidd)):
        ...
        lpr_v ['text'] = 'Генерация видео...    ' + qq [0] + '.' + qq [1].ljust (3, '0') + '%    ( ' + str (q + 1) + ' / ' + str (len (vidd)) + ' )'
...
scale1 = tkinter.Scale (image_window, orient = tkinter.HORIZONTAL, length = 230, from_ = 0, to = 12, bg = color1, variable = mxxi, activebackground = color1, command = set_mxi)
scale1.set (2)
...
nnn1 = tkinter.Label (image_window, width = 39, bg = color2, text = 'Количество итераций:', anchor = 'w')
nnn1.place (x = www + 40, y = 400)

scale2 = tkinter.Scale (image_window, orient = tkinter.HORIZONTAL, length = 230, from_ = -4, to = 2, bg = color1, variable = biig, activebackground = color1, command = set_big)
scale2.set (-3)
...
nnn2 = tkinter.Label (image_window, width = 39, bg = color2, text = 'Разрешение:', anchor = 'w')
nnn2.place (x = www + 40, y = 450)

lab = tkinter.Label (image_window, bg = color2, text = 'x: ' + str ((width * 0.5 - (cx / big)) * 4 / width) + '\ny: ' + str ((height * 0.5 - (cy / big)) * 4 / height) + '\n\nzoom: ' + str (zoom), font = 'Courier 13', just = 'left')
lab.place (x = www + 40, y = 20)

svbt = tkinter.Button (image_window, text = 'Сохранить изображение', height = 2, width = 38, bg = color1, activebackground = color1, command = lambda: save_image (imgag))
svbt.place (x = www + 40, y = 160)

upbt = tkinter.Button (image_window, text = 'Обновить изображение', height = 2, width = 38, bg = color1, activebackground = color1, command = modify_img)
...
if 'video.txt' in os.listdir ('.'):
    ...
    if zzz [0] == 'True':
        ...
        if flag:
            vibt = tkinter.Button (image_window, text = 'Создать видео', height = 2, width = 38, bg = color3, activebackground = color3, command = lambda: video (zzz [3::]))
            vibt.place (x = www + 40, y = 260)

            image_window.title ('Фрактал Мандельброта    ' + VERSION + '    Vitmalok    - Режим видео')
